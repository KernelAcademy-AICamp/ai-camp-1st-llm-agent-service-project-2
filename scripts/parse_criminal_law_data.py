...
def main():
    ...
    # 데이터 디렉토리 (환경변수로 오버라이드 가능)
    default_data_dir = BASE_DIR / "data" / "raw" / "04.형사법 LLM 사전학습 및 Instruction Tuning 데이터" / "3.개방데이터" / "1.데이터" / "Training" / "01.원천데이터"
    DATA_DIR = Path(os.getenv("CRIMINAL_LAW_DATA_DIR", str(default_data_dir)))

    # 출력 파일
    OUTPUT_FILE = BASE_DIR / "data" / "parsed" / "criminal_law_chunks.jsonl"
