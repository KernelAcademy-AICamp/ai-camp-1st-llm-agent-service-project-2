...
**프로젝트 상태**: ✅ **Phase 01 완료** - 웹 MVP 개발 완료 (로컬 환경)
**다음 마일스톤**: 프로덕션 배포 + Phase 02 준비

---

## ✅ Phase 01 완료: 웹 MVP

> **웹 버전 개발 완료 (로컬 환경)**, 실무 변호사 워크플로우 중심의 3가지 핵심 기능 구현
...
#### 3️⃣ 문서 작성 어시스트
- 📝 **10개 템플릿 제공** (소장, 답변서, 고소장, 변론요지서, 내용증명, 손해배상청구서 + **계약서 4종**)
- 🤖 AI 자동 생성 (판례 인용 + 법리 설명 + 스마트 변수 추출)
- 💾 TXT 다운로드 (DOCX/PDF 변환 지원)
...
### 2. 검증된 RAG 시스템
- ✅ **현재: 388,767개** 형사법 문서 (통합 데이터베이스)
  - 판례: 378,525개 (대법원, 고등법원, 지방법원)
  - 법령: 9,442개 (형법, 형사소송법, 특별법)
  - 해석례: 800개 (법제처 유권해석)

- ✅ **데이터 통합 완료**
  - HuggingFace 데이터셋 통합 완료 (348,000개 추가)
  - 현재 388,767개 문서로 목표 달성
  - ChromaDB 인덱싱 완료 (3.9GB)

- 🔄 **데이터 자동 업데이트 시스템**
  - **일일 크롤링**: 국가법령정보센터 Open API (✅ 작동 중)
    - 출처: [국가법령정보 공동활용](https://open.law.go.kr/)
    - 매일 최신 판례 10건 자동 수집
    - Playwright 동적 크롤링 + 증분 인덱싱
  - **주간 맞춤 크롤링**: 사용자 피드백 기반
    - 인기 키워드별 판례 수집 (주 1회)
    - 실시간 데이터 최신성 유지
...
**계약서 (4개) 🆕**
7. **근로계약서** - 20개 변수 (급여, 근무시간, 당사자 정보)
8. **임대차계약서** - 32개 변수 (보증금, 임대차 기간, 특약사항)
9. **업무위탁계약서** - 32개 변수 (수수료, IP권리, 비밀유지)
10. **매매계약서** - 28개 변수 (매매대금, 계약금/잔금, 부동산 정보)

#### 🤖 **스마트 생성 기능**

- **8가지 시나리오 자동 감지**
  - AI가 업로드된 문서를 분석하여 적합한 시나리오 판단
  - 시나리오별 추천 템플릿 자동 제안
  - 신규: 시나리오 6 "계약서 검토/작성" - 4개 계약서 템플릿 자동 연결

- **AI 변수 추출** (70% 자동화)
  - 사건 분석 결과에서 당사자, 날짜, 금액 자동 추출
  - 사용자는 추출되지 않은 필드만 입력
  - 금액 자동 포맷팅 (천단위 콤마)

- **템플릿 검증**
  - Unfilled placeholder 자동 감지 및 경고
  - 필수 필드 누락 시 생성 차단
  - 생성 후 즉시 다운로드 가능 (TXT 형식)
...
| **HuggingFace** | 348,000개 | 국가법령정보센터 판례 커뮤니티 데이터 | 정적 | ✅ 통합 완료 |
| **Open API 크롤링** | 증분 수집 | 실시간 최신 판례 | 일일/주간 | ✅ 작동 중 |
| **현재 총계** | **388,767개** | **통합 데이터베이스** | **자동 업데이트** | ✅ 달성 |
...
- [x] 388,767개 문서 임베딩 및 인덱싱
- [x] Constitutional AI 적용 (6 Principles + Self-Critique)
- [x] 기본 UI (React + FastAPI)
- [x] **OpenAI/Claude API 통합** ✅
- [x] **파일 업로드 + AI 분석** ✅
- [x] **문서 작성 템플릿 (10개)** ✅
- [x] **사용자 피드백 시스템 (SQLite)** ✅
- [x] **자동화 크롤링 (APScheduler + Playwright)** ✅
- [x] **통합 테스트 (로컬)** ✅
- [ ] **프로덕션 배포** 🔄 (예정)

📋 [Phase 01 상세 계획](./MVP_PHASE_01.md)

### Phase 02: 프로덕션 배포 + Desktop 앱 (3-4개월)
**목표**: 클라우드 배포 + Electron 앱 + 로컬 LLM + 전문 분야 특화

#### 🎯 **우선순위 1: 프로덕션 배포** 🆕

- [ ] **클라우드 배포**
  - [ ] 플랫폼 선택 (AWS / Vercel / Railway / Netlify)
  - [ ] 백엔드 배포 (FastAPI + ChromaDB)
  - [ ] 프론트엔드 배포 (React)
  - [ ] 도메인 연결 및 HTTPS 설정
  - [ ] 환경변수 및 시크릿 관리
  - [ ] CI/CD 파이프라인 구축
...
- [ ] **데이터베이스 마이그레이션**
  - [ ] SQLite → PostgreSQL 전환 (피드백 시스템)
  - [ ] ChromaDB 클라우드 호스팅 또는 영구 스토리지 설정
  - [ ] 데이터 백업 및 복구 전략
...
#### 🎯 **우선순위 2: Desktop 앱 + QDoRA Adapter**
...
- [ ] **QDoRA Adapter 시스템**
  - [ ] 교통사고 전문 Adapter (5,000개 판례 학습)
  - [ ] 형사 일반 Adapter (8,000개 판례)
  - [ ] 민사 Adapter (6,000개 판례)
  - [ ] Adapter 자동 교체 (재시작 불필요)

- [ ] **RAG 성능 최적화**
  - [ ] Learning to Rank (사용자 피드백 기반)
  - [ ] 개인화 추천 시스템
  - [ ] 검색 속도 개선 (< 0.5초)
...
**프로젝트 상태**: ✅ **Phase 01 완료** (웹 MVP 개발 완료)
**현재 환경**: 로컬 개발 서버 (localhost:3000)
**다음 단계**: 프로덕션 배포 (AWS/Vercel) + Phase 02 준비
**주요 성과**:
- 388,767개 문서 RAG 시스템 구축
- 10개 법률 문서 템플릿 (소송 6개 + 계약서 4개)
- Constitutional AI 환각 70% 감소
- 사용자 피드백 시스템 (SQLite)
- 자동 크롤링 시스템 (APScheduler + Playwright)
